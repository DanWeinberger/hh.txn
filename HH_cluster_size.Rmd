...
SAR.ratio.samples <- (SAR1.samples / SAR2.samples)
...
## Simulate data from model with specified vaccine effect

```{r}
source('./R/longini_sim.R')

#sET THE VACCINE EFFECTS
set.RR.community <- 0.6
set.RR.sar <- 0.3

beta1 <- log(set.RR.community)
delta1 <- log(set.RR.sar)

baseline.community.prob <- 0.15 
baseline.HH.prob <- 0.15
set.max.hh.size <- 8

simN <- longiniSim.m( 
                     max.hh.size=set.max.hh.size, 
                     N.households=500,
                     baseline.community.prob = 0.15 ,
                     baseline.HH.prob = 0.15,
                     RR.community=set.RR.community,
                     RR.sar=set.RR.sar,
                     sizes=rep(50, set.max.hh.size)
                     )
...
choose_kj_mat <- matrix(0, nrow=dim(simN)[2], ncol=dim(simN)[2])
for(k in 2: dim(simN)[2]){
  for(j in 1:k){
    choose_kj_mat[k,j] <- choose(k,j)
  }
}
...
model_jags_covar<-jags.model(model_spec, 
                       ...
                       data=list( 
                                 'ds'=simN,
                                 'max.hh.size'=dim(simN)[2],
                                 'n.hh'= apply(simN,c(2,3),sum),
                                 'choose_kj_mat'=choose_kj_mat
                                 ),
                       n.adapt=10000, 
                       n.chains=3)
...
CPI.ratio.samples <- CPI2.samples / CPI1.samples
...
#Ratio of SAR for HH with and without vaccine 
SAR1.samples <- 1 - posterior_samples.covar.all[,Q1.index]
SAR2.samples <- 1 - posterior_samples.covar.all[,Q2.index] 
