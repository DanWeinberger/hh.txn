...
chain_bin_lik <- function(params,Y,X){
  ...
  X1 <- X[X$hhID==i,]
  Y1 <- Y[Y$hhID==i,]
  Y1 <- setorder(Y1, t.index,ID,hhID)
  
  #### Define logit_p = X*params; need  to add as.matrix
  logit_p <- as.vector(as.matrix(X1[,1:4]) %*% params) ## Added as.matrix
  ...
  data_tab <- cbind.data.frame(log.q=log(q),X1)
  ...
  ll.piece= sum(dbinom(x=Y1$A,size=1,prob = pi,log = TRUE),na.rm = TRUE)
  ll <- ll + ll.piece
  }
  return(-ll)
}
