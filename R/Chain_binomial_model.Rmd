...
#Generate random draws for duration of infectious, 
  ...
  hh_df_rep$expose.dist= rgamma(n.sim,4,  0.75) #duration latent
  hh_df_rep$infect.dist= rgamma(n.sim,4, 0.75) #duration infectiousness
  hh_df_rep$end.inf= rgamma(n.sim,4, 0.75) #duration infectiousness
  ...
  hh_df_rep.spl <- lapply(hh_df_rep.spl, function(x){
           ...
           x$day.exposed1 <- round(x$day.infectious - x$expose.dist)+1
...
model_jags<-jags.model(model_spec, 
                       ...
                       inits=list(inits1),                                           
                       data=list('N.HH'=nrow(infected_matrix),                                                                               'N.hh.members'= N.hh.members,
                                 'vaxdose1'=hh_df_rep.c['vax1dose',,,1],                                   
                                   'day.infectious'=hh_df_rep.c['day.infectious',,,],
                                 
                                 'day.exposed'=hh_df_rep.c['day.exposed',,,]+1,
                                                                    'day.infectious.end'=hh_df_rep.c['day.infectious.end',,,],
                                 'y' = t(1-hh_df_rep.c['infected',,,1]),
                                 'y2' = t(1-hh_df_rep.c['infected',,,1]),
                                 'selecter1'=1,
