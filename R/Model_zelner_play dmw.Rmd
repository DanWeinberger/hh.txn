...
expose.dist <- array(NA, dim=c(N.samples,N.HH,max(N.hh.members+1) ))
   ...
   #Generate random draws for duration of infectious, 
    infect.dist <-  
      array(rgamma(n.sim,sh1, ra1), dim=c(N.HH,max(N.hh.members), N.samples)) #duration infectiousness
    ...
    max.time.hh <- round(apply(expose.dist,c(1,3),max, na.rm=T) +
      apply(infect.dist,c(1,3),max, na.rm=T) )
   ...
   last.day.infect.hh <- apply(day.infectious, c(1,3), max, na.rm=T)
