...
delay.gen.simplified <- function(input_df){
  ...
  data_tab <- cbind.data.frame(df4[,c('infect.at.timet','ID','hhID','t.index','vax1dose')])
  data_t = data.table(data_tab)
  Y.df = data_t[,list(A = mean(infect.at.timet)), by = 'ID,hhID,t.index,vax1dose']
