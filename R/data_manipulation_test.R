####### Convert dataframes to datatables before merging
delay.gen.dtable <- function(input_df){
  ...
  df3 <- merge(df1b,df2,by.y='hhID_b' , by.x='hhID',allow.cartesian=TRUE)
...
########  Run this function for one HHs at a time
delay.gen.singleHH <- function(input_df){
  
  #df1 <- do.call('rbind.data.frame',df1.ls)
  df1 <- delay_dist_sim(input_df)
  #keep just variables we need
  df1b <- df1[, c('ID','hhID','vax1dose','vax2dose','infected','day.exposed','day.infectious.end','day.infectious','max.time.hh')]
