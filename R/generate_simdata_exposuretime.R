...
gen.hh <- function(idN, prob.hh.txn=0.5){
  ...
  index_random2 <- rbinom(0:HH.size,HH.size,prob=0.5)
  ...
  df1$day_index <- 21 + round(as.numeric(df1$date.test - min(df1$date.test)))
  ...
  #day.matrix=day of test for the person
    day.infectious[j] <- df1$day_index[j] - infect.dist[j]  #infectious prior to test
    day.exposed[j] <- day.infectious[j] - expose.dist[j]  #latent period
    day.infectious.end[j] <- day.infectious[j] + end.inf[j]  #how long infectious after you become infectious
    ###
  }
  
  ### Define index case:
  for (j in 1:length(df1$ID)){
    if(min(day.exposed)>=day.exposed[j])
      df1$index.case[j] <- 1 #indicator for whether this person is an index
    else{
      df1$index.case[j] <- 0
    }
  }
