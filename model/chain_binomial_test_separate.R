  ...
  for(j in 1:N.hh.members[i]){ 
   ...
   # y[i,j] ~ dbern(q[i,j]) #(Person j in HH i: 1= uninfected; 0= infected)
    ...
    for(t in 1:time.study.HH[i, selecter1]){
      ...
      d[i,j,(N.hh.members[i] + 1),t] <- step( t > day.infectious[i,(N.hh.members[i] + 1),selecter1]+0.5)*step( t < min(day.exposed[i,j,selecter1], day.infectious.end[i,(N.hh.members[i] + 1),selecter1]) + 0.5)
    ...
    prob_uninf_to_timej[i,j] <- exp(sum(log.prob_uninf[i,j,,1:(day.exposed[i,j,selecter1]-1)]))
