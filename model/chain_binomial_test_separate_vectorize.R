...
for(i in 1:N.obs){ 
    ...
    y[i] ~ dbern(like[i]) #(Person j in HH i: 1= uninfected; 0= infected)

    for(t in 1:time.study.HH[i, selecter1]){
        log.p[i,(N.hh.members[i]+1),t] <- delta0 + beta1*vaxdose1[i]  #outside HH infection
     
      for(k in 1:N.hh.members[i]){ #k is different from j
        d[i,k,t] <- step(abs(j-k)-0.5)*step( t > day.infectious[i,k,selecter1]+0.5)*step( t < min(day.exposed[i,j,selecter1], day.infectious.end[i,k,selecter1]) + 0.5)
        log.prob_uninf[i,j,k,t] <- step(abs(j-k)-0.5)*log((1-p[i,j,k,t])^(d[i,j,k,t]))
        log.p[i,j,k,t] <- alpha0 + beta1*vaxdose1[i]  + kappa1*vaxdose1[i,k] 
        p[i,k,t] <- exp(log.p[i,k,t])
